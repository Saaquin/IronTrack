...
impl FlightLine {
    ...
    pub fn to_datum(
        ...
        let (lats, lons, elevations, source_datum, is_transit) = self.into_parts();

        // Phase 1: Convert current elevations to WGS84 ellipsoidal (h).
        let ellipsoidal_h: Vec<f64> = match source_datum {
            AltitudeDatum::Wgs84Ellipsoidal => elevations,
            AltitudeDatum::Egm2008 => lats
                .iter()
                .zip(lons.iter().zip(elevations.iter()))
                .map(|(&la, (&lo, &h))| {
        ...
        let final_elevations: Vec<f64> = match target {
            ...
            AltitudeDatum::Egm2008 => lats
                .iter()
                .zip(lons.iter().zip(ellipsoidal_h.iter()))
                .map(|(&la, (&lo, &h))| {
                    let n = egm2008.undulation(la.to_degrees(), lo.to_degrees())?;
                    Ok(h - n)
                })
